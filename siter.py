...
class WheelBuilder:
    ...
    def _read_installer(
        ...
        if installer_file.exists():
            return (
                installer_file.read_text().strip()
            )
        return "unknown"

    def _find_scripts_for_package(
        self, package_name: str, records: Dict
    ) -> List[Path]:
        """Find console scripts for this package"""
        if (
            not self.bin_dir
            or not self.bin_dir.exists()
        ):
            return []
        ...
        if (
            ...
            for (
                ...
                entry_points_file.read_text().splitlines()
            ):
                line = line.strip()
    ...
    def _find_data_for_package(
        self, package_name: str, records: Dict
    ) -> List[Tuple[Path, str]]:
        """Find data files in share/ for this package"""
        if (
            not self.share_dir
            or not self.share_dir.exists()
        ):
            return []
        ...
        for item in self.share_dir.rglob("*"):
            ...
            if any(
                pkg_normalized in p.name.lower()
    ...
    def _get_wheel_tags(
        ...
        try:
            ...
            python_tag = (
                f"cp{py_ver.major}{py_ver.minor}"
            )
            abi_tag = python_tag
    ...
    def build_wheel(
        ...
        if len(parts) < 2:
            log.warning(
                f"Invalid dist-info name: {dist_info_dir.name}"
            )
            return None
        ...
        log.info(
            f"Building wheel for {pkg_name} {version}"
        )
        ...
        data_files = self._find_data_for_package(
            pkg_name, records
        )

        if scripts:
            log.info(
                f"  Found {len(scripts)} script(s)"
            )
        if data_files:
            log.info(
    ...
    def build_all(self) -> int:
        ...
        dist_infos = sorted(
            self.site_packages.glob("*.dist-info")
        )

        if not dist_infos:
            log.error(
                f"No packages found in {self.site_packages}"
            )
            return 0

        log.info(
            f"Found {len(dist_infos)} package(s) in {self.site_packages}"
        )
        if self.venv_root:
            log.info(
                f"Virtual environment: {self.venv_root}"
            )
        ...
        for dist_info in dist_infos:
            try:
                ...
                log.error(
                    f"Failed to build {dist_info.name}: {e}"
                )
...
def find_site_packages() -> List[Path]:
    ...
    for pattern in [".venv", "venv", "env"]:
        for venv in cwd.rglob(pattern):
            if venv.is_dir():
                for sp in venv.rglob(
                    "site-packages"
                ):
                    if (
                        sp.is_dir()
                        and sp not in candidates
                    ):
...
Examples:
  ...
  %(prog)s --package requests
    ...
    if args.list:
        ...
        print(
            f"Found {len(sites)} site-packages:"
        )
    ...
    if args.site_packages:
        ...
        if not site_packages.exists():
            log.error(
                f"Site-packages not found: {site_packages}"
            )
            return 1
    else:
        sites = find_site_packages()
    ...
    log.info(
        f"Using site-packages: {site_packages}"
    )

    # Build wheels
    builder = WheelBuilder(
        site_packages, args.output
    )
    ...
    if args.package:
        ...
        if not matches:
            log.error(
                f"Package not found: {args.package}"
            )
            return 1

        if len(matches) > 1:
            log.warning(
                f"Multiple matches for '{args.package}':"
            )
            for m in matches:
                log.warning(f"  - {m.name}")
            log.info(f"Building all matches...")
