...
class PackageRepacker:
    def __init__(
        self, output_base: str = "~/tmp/repack"
    ):
    ...
    def find_site_packages_dirs(
        ...
        for search_path in search_paths:
            ...
            try:
                ...
                for site_pkg in search_path.rglob(
                    "site-packages"
                ):
                    if (
                        site_pkg.is_dir()
                        ...
                        logger.info(
                            f"Found site-packages: {site_pkg}"
                        )

                # Look for dist-packages (Debian/Ubuntu)
                for dist_pkg in search_path.rglob(
                    "dist-packages"
                ):
                    if (
                        dist_pkg.is_dir()
                        ...
                        logger.info(
                            f"Found dist-packages: {dist_pkg}"
                        )
                ...
                logger.debug(
                    f"Permission denied scanning {search_path}: {e}"
                )
    ...
    def create_wheel_structure(
        ...
        if is_pure_python:
            ...
            try:
                ...
                best_tag = next(
                    sys_tags()
                )  # Get the first, most specific tag
        ...
        for (
            ...
            files
        ):  # file_path is already the relative path from site-packages
            ...
            if source_path.exists():
                ...
                logger.warning(
                    f"File from RECORD not found at {source_path}"
                )
        ...
        original_metadata = (
            original_dist_info_dir / "METADATA"
        )
        new_metadata_path = (
            dist_info_dir / "METADATA"
        )
        if (
            not new_metadata_path.exists()
            and original_metadata.exists()
        ):
            shutil.copy2(
                original_metadata,
                new_metadata_path,
    ...
    def copy_package_files(
        ...
        try:
            ...
            metadata = self.get_package_info_from_dist_info(
                dist_info_dir
            )
            ...
            is_pure_python = True
            with open(
                record_file, "r", encoding="utf-8"
            ) as f:
                for line in f:
                    file_path_str = line.split(
                        ","
                    )[0].strip()
                    # Include all files from RECORD, except the RECORD file itself
                    if (
                        file_path_str
                        and not file_path_str.endswith(
                            ".dist-info/RECORD"
                        )
                    ):
            ...
            if not files_to_include:
                logger.warning(
                    f"No files found for package {package_name}"
                )
                return False
    ...
    def copy_all_packages(self):
        ...
        for (
            site_packages_dir
        ) in self.found_site_packages:
            logger.info(
                f"Processing site-packages: {site_packages_dir}"
            )
            ...
            try:
                ...
                if (
                    ...
                    env_name = (
                        site_packages_dir.parent.parent.name
                    )
            except Exception:
                pass  # stick with 'local_env'

            output_dir = (
                self.output_base / env_name
            )
            output_dir.mkdir(
                parents=True, exist_ok=True
            )
        ...
        logger.info(
            f"Total packages copied: {total_copied}"
        )
        logger.info(
            f"Package files saved to: {self.output_base}"
        )
...
def main():
    ...
    try:
        ...
        if args.skip_scan:
            ...
            site_packages_dirs = (
                repacker.find_site_packages_dirs()
            )
            logger.info(
        ...
        if not repacker.found_site_packages:
            logger.error(
                "No site-packages directories found!"
            )
            return 1
